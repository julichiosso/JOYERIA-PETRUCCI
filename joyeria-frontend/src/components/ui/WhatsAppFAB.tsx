...
export default function WhatsAppFAB({ href }: WhatsAppFABProps) {
  const targetHref = href || "https://wa.me/5493406419736?text=Hola%20Petrucci,%20quisiera%20hacer%20una%20consulta%20personalizada.";

  return (
    <div className="fixed bottom-6 right-5 z-40 flex items-center group">
      {/* Pulsing Glow Ring */}
      ...
      <motion.a
        ...
        whileTap={{ scale: 0.95 }}
        className="relative flex items-center justify-center w-14 h-14 rounded-full bg-[#25D366] text-white shadow-xl hover:bg-[#20bd5a] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#25D366] transition-all duration-300"
      >
        ...
        <span
          className="absolute right-16 bg-[#111111] text-white text-[11px] font-body tracking-wider uppercase py-2 px-3.5 shadow-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none"
          role="tooltip"
        >
          Atención Boutique • WhatsApp
        </span>
      </motion.a>
    </div>
  );
